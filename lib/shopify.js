...
function generateMessage(params, delimiter) {
    var parameters = [];
    ...
    return parameters.sort().join(delimiter);
}

function encryptMessage(message, secret, encoding) {
...
ShopifyAPI.prototype.is_valid_proxy_request = function(params) {
    var message = generateMessage(params, "");
    var digest = encryptMessage(message, this.config.shopify_shared_secret);
    return (digest === params['signature']);
};

ShopifyAPI.prototype.is_valid_webhook = function(headers, body) {
    var digest = encryptMessage(body, this.config.shopify_shared_secret, "base64");
    return (digest === headers['x-shopify-hmac-sha256']);
};
